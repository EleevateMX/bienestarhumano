...
/* ============================================================
   HISTÓRICO MENSUAL SIMULADO (septiembre 2024 → diciembre 2025)
   Estos valores son referencia para que las gráficas de tendencias
   tengan contexto. Se reemplazan al cargar meses reales.
   Los meses enero-abril 2026 se generan a partir del REPORT_2026_Q1.
   ============================================================ */
function generateMonthlyHistory(){
  const months = [];
  const start = new Date(2024, 8, 1);   // septiembre 2024
  const end   = new Date(2025, 11, 31); // diciembre 2025
  const avg = { medicos: 5713, odonto: 2383, enfermeria: 9897, rehab: 1476, mental: 1402, nutri: 838 };
  const cur = new Date(start);
  let i = 0;
  while(cur <= end){
    const y = cur.getFullYear(), m = cur.getMonth();
    // Serie determinística: evita que PC y teléfono generen cantidades distintas.
    ...
    months.push({
      ...
      uploadedBy: 'Histórico de referencia',
      uploadedAt: cur.toISOString(),
      summary: {
        medicos: Math.round(avg.medicos * seasonal * trend),
        odonto: Math.round(avg.odonto * seasonal * trend),
        enfermeria: Math.round(avg.enfermeria * seasonal * trend),
        rehab: Math.round(avg.rehab * seasonal * trend),
        mental: Math.round(avg.mental * seasonal * trend),
        nutri: Math.round(avg.nutri * seasonal * trend)
      }
    });
    i++;
