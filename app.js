...
function renderTrends(){
  ...
  // Verificar que el contenedor esté visible y dimensionado
  const trendCard = $('#trendChart');
  if(!trendCard) return;
  const w = trendCard.parentElement.clientWidth;
  if(w === 0){
    // El contenedor todavía no se midió. Reintenta en el siguiente frame.
    return requestAnimationFrame(renderTrends);
  }
  ...
  // ----- Gráfica principal: atenciones totales por mes -----
  const ctxT = $('#trendChart').getContext('2d');
  if(STATE.charts.trend) STATE.charts.trend.destroy();
  const grad = ctxT.createLinearGradient(0, 0, 0, 320);
  grad.addColorStop(0, 'rgba(141,198,63,.55)');
