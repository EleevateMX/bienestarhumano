...
const STORAGE_KEY = 'bh_merida_v5_estable';
...
function openModuleDetail(moduleId){
  ...
  modal.innerHTML = `
    ...
    <div class="module-modal-card">
      ...
      <div class="module-modal-head">
        <span>${isMental(mod)?'AlmaNova · Psicología':'Módulo de Salud'}</span>
      ...
      <div class="module-modal-grid">
        ...
        <div class="chart-card"><div class="chart-head"><h3>Servicio más usado</h3></div><div class="chart-body" data-chart="moduleServiceChart"></div></div>
      </div>
      <div class="module-month-list">
        ${latest.map(p=>{
          const m=p.modules[moduleId]||{};
          const top=SERVICE_DEFS.map(s=>({label:s.label,val:m[s.key]||0})).sort((a,b)=>b.val-a.val)[0];
          return `<div><strong>${p.label}</strong><span>${fmt(totalOf(m))} atenciones</span><small>Más usado: ${top.label} (${fmt(top.val)})</small></div>`;
        }).join('')}
      </div>
    </div>`;
  ...
  nativeBars('moduleServiceChart', SERVICE_DEFS.map(s=>s.label), SERVICE_DEFS.map(s=>serviceTotals[s.key]), SERVICE_DEFS.map(s=>s.color));
}
...
function drawMapMarkers(){
  ...
  activeModules().forEach(mod=>{
    ...
    const marker = L.marker([mod.lat,mod.lng], { icon:L.divIcon({ className:'', html:`<div class="module-marker ${isMental(mod)?'mental':''}">${mod.name.substring(0,2).toUpperCase()}</div>`, iconSize:[32,32], iconAnchor:[16,16] }) });
...
function renderDataLoader(){
  ...
  activeModules().forEach(mod => {
    ...
    tr.innerHTML = `<td class="module-name-cell">${escapeHtml(mod.name)}</td>${SERVICE_DEFS.map(s=>`<td><input type="number" min="0" data-svc="${s.key}" value="0" /></td>`).join('')}<td class="total-cell" data-total>0</td>`;
...
function initNav(){
  ...
  $('#logoutBtn')?.addEventListener('click',()=>{ STATE.currentUser=null; STATE.selectedUser=null; $('#password') && ($('#password').value=''); $$('.user-chip').forEach(c=>c.classList.remove('selected')); $('#app')?.classList.add('hidden'); $('#login')?.classList.remove('hidden'); });
