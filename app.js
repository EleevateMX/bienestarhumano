...
const STORAGE_KEY = 'bh_merida_v14_data_fix';
...
function createDistributedPeriod(year, month, modulesSrc, fraction, fullSummarySrc=null){
  ...
  // El resumen puede incluir programas de salud que no son sedes visibles
  // (médico a domicilio, ferias, CEMANUD, unidades móviles, etc.).
  // Las sedes se conservan para cards/mapa, pero los KPI usan el concentrado completo.
  const summaryBase = fullSummarySrc || aggregateModuleSummary(modulesSrc || {});
  const summary = { medicos:0, odonto:0, enfermeria:0, rehab:0, mental:0, nutri:0 };
  svcKeys().forEach(k => summary[k] = Math.round((Number(summaryBase[k])||0) * fraction));
  return { period, label, uploadedBy:'Datos iniciales', uploadedAt:new Date().toISOString(), modules, summary };
}
...
function initData(){
  ...
  if(stored && Array.isArray(stored.periods) && stored.periods.length){
    ...
    history.push(
      createDistributedPeriod(2026,0,REPORT_2026_Q1.modules,.21,REPORT_2026_Q1.fullSummary),
      createDistributedPeriod(2026,1,REPORT_2026_Q1.modules,.24,REPORT_2026_Q1.fullSummary),
      createDistributedPeriod(2026,2,REPORT_2026_Q1.modules,.26,REPORT_2026_Q1.fullSummary),
      createDistributedPeriod(2026,3,REPORT_2026_Q1.modules,.29,REPORT_2026_Q1.fullSummary)
    );
...
function initLogin(){
  const setLoginError = (msg='') => { const el = $('#loginError'); if(el) el.textContent = msg; };

  const selectUser = (chip) => {
    if(!chip) return;
  ...
  const attemptLogin = (e) => {
    ...
    const selectedChip = $('.user-chip.selected');
    const user = STATE.selectedUser || selectedChip?.dataset.user || '';
    const pass = ($('#password')?.value || '').trim();

    if(!user){ setLoginError('Selecciona un usuario'); return false; }
    if(!pass){ setLoginError('Ingresa tu contraseña'); return false; }
  ...
  $$('.user-chip').forEach(chip=>{
    ...
    chip.addEventListener('touchend',(e)=>{ e.preventDefault(); selectUser(chip); }, {passive:false});
  });

  $('#loginForm')?.addEventListener('submit', attemptLogin);
  $('.btn-login')?.addEventListener('click', attemptLogin);
  $('#password')?.addEventListener('keydown', e=>{ if(e.key === 'Enter') attemptLogin(e); });
  $('#password')?.addEventListener('input',()=>setLoginError(''));
}
function onLoggedIn(){
...
/* ============================================================
   V13 · Todas las sedes + editor completo de información y métricas
   ============================================================ */
function uiStateV13(){
  const st = uiStateV12();
...
function metricKeyV13(program, id, period){ return `${program}:${id}:${period || (getCurrentPeriod()?.period || 'all')}`; }
function getMetricOverrideV13(program, id, period){ return uiStateV13().metrics[metricKeyV13(program,id,period)] || null; }
function saveMetricOverrideV13(program, id, period, metrics){
  const st = uiStateV13();
  st.metrics = st.metrics || {};
  st.metrics[metricKeyV13(program,id,period)] = Object.assign({}, metrics, {updatedAt:new Date().toISOString()});
  saveUiStateV12(st);
}
function applyMetricOverridesV13(periodObj){
  if(!periodObj || STATE.currentProgram !== 'salud') return periodObj;
  const st = uiStateV13();
  const period = periodObj.period || 'all';
  const modules = Object.assign({}, periodObj.modules || {});
  const summary = Object.assign({ medicos:0, odonto:0, enfermeria:0, rehab:0, mental:0, nutri:0 }, periodObj.summary || aggregateModuleSummary(modules));
  Object.keys(st.metrics || {}).forEach(key=>{
    const [program, id, p] = key.split(':');
    if(program !== 'salud' || p !== period) return;
    const override = st.metrics[key] || {};
    const before = modules[id] || {};
    const after = Object.assign({}, before, override);
    modules[id] = after;
    // Ajusta el resumen con la diferencia del módulo editado sin perder
    // los acumulados globales que no están ligados a una sede visible.
    svcKeys().forEach(k => {
      const delta = (Number(after[k]) || 0) - (Number(before[k]) || 0);
      summary[k] = (Number(summary[k]) || 0) + delta;
    });
  });
  return Object.assign({}, periodObj, {modules, summary});
}
const _getCurrentPeriodV13 = getCurrentPeriod;
getCurrentPeriod = function(){ return applyMetricOverridesV13(_getCurrentPeriodV13()); };

function metricFieldsV13(program, id, item){
  if(program !== 'salud') return '';
  const cur = getCurrentPeriod();
  const data = Object.assign({}, cur.modules?.[id] || {}, getMetricOverrideV13(program,id,cur.period) || {});
  const fields = (isMental(item) ? SERVICE_DEFS.filter(s=>s.key==='mental') : SERVICE_DEFS).map(s=>`
    <label>${escapeHtml(s.label)}<input name="metric_${s.key}" type="number" min="0" step="1" value="${Number(data[s.key])||0}"></label>
  `).join('');
  const label = cur.period === 'all' ? 'Valores acumulados visibles' : `Valores de ${cur.label}`;
  return `<div class="edit-section-title">${escapeHtml(label)}</div>${fields}`;
}
function educationFieldsV13(item){
  return educationCategories().map(c=>`
    <label class="edit-full">${escapeHtml(c.label)}<textarea name="edu_${c.key}" rows="3">${escapeHtml(item[c.key] || '')}</textarea></label>
  `).join('');
}
function sportsFieldsV13(item){
  const activitiesText = item.activitiesText || (Array.isArray(item.activities) ? item.activities.map(a=>[a.disciplina,a.responsable,a.horario,a.edades,a.cupo,a.categoria].filter(Boolean).join(' | ')).join('\n') : '');
  return `
    <label>Responsable / administrador<input name="admin" value="${escapeHtml(item.admin || item.responsable || '')}"></label>
    <label>Tipo / categoría<input name="tipo" value="${escapeHtml(item.tipo || item.categoria || item.type || '')}"></label>
    <label class="edit-full">Actividades / horarios / comités<textarea name="activitiesText" rows="7">${escapeHtml(activitiesText)}</textarea></label>
  `;
}
function openEditModalV12(program, id){
  const base = catalogItemV12(program, id); if(!base) return;
  ...
  const nameVal = item.name || item.sede || '';
  const addressVal = item.address || item.colony || item.zona || item.tipo || '';
  const detailVal = item.description || item.observaciones || item.schedule || item.note || item.admin || item.responsable || '';
  const extraFields = program === 'salud' ? metricFieldsV13(program,id,item) : program === 'educacion' ? educationFieldsV13(item) : program === 'deporte' ? sportsFieldsV13(item) : '';
  let modal = $('#editInfoModalV12');
  if(!modal){ modal = document.createElement('div'); modal.id='editInfoModalV12'; modal.className='module-modal hidden'; document.body.appendChild(modal); }
  modal.innerHTML = `
    <div class="module-modal-backdrop" data-close="1"></div>
    <form class="module-modal-card edit-info-card edit-info-card-wide" id="editInfoFormV12">
      <button type="button" class="module-modal-close" data-close="1">×</button>
      <div class="module-modal-head"><span>Gestión de información · ${escapeHtml(programLabel(program))}</span><h2>Editar información completa</h2><p>Puedes cambiar textos, totales visibles y campos de gestión. La ubicación del mapa no se altera.</p></div>
      <div class="edit-grid-v12">
        <label>Nombre visible<input name="name" value="${escapeHtml(nameVal)}" required></label>
        <label>Dirección / zona / tipo<input name="address" value="${escapeHtml(addressVal)}"></label>
        <label>Código mapa<input name="shortCode" value="${escapeHtml(item.shortCode || (program==='salud' && isMental(item) ? 'AN' : ''))}" placeholder="Ej. AN, BT, UD"></label>
        <label>Horario / responsable corto<input name="schedule" value="${escapeHtml(item.schedule || item.admin || item.responsable || '')}"></label>
        <label class="edit-full">Información, horarios u observaciones<textarea name="detail" rows="5">${escapeHtml(detailVal)}</textarea></label>
        ${extraFields}
      </div>
      <div class="edit-actions-v12">
  ...
  $('#editInfoFormV12')?.addEventListener('submit', e => {
    ...
    const fd = new FormData(e.currentTarget);
    const data = {
      name:fd.get('name'), sede:fd.get('name'), address:fd.get('address'), colony:fd.get('address'), zona:fd.get('address'),
      shortCode:fd.get('shortCode'), schedule:fd.get('schedule'), admin:fd.get('schedule'), responsable:fd.get('schedule'),
      description:fd.get('detail'), observaciones:fd.get('detail'), note:fd.get('detail')
    };
    if(program === 'educacion') educationCategories().forEach(c=> data[c.key] = fd.get(`edu_${c.key}`) || '');
    if(program === 'deporte'){
      data.admin = fd.get('admin') || data.admin;
      data.responsable = fd.get('admin') || data.responsable;
      data.tipo = fd.get('tipo') || data.tipo;
      data.categoria = fd.get('tipo') || data.categoria;
      data.type = fd.get('tipo') || data.type;
      data.activitiesText = fd.get('activitiesText') || '';
    }
    if(program === 'salud'){
      const metrics = {};
      SERVICE_DEFS.forEach(s=>{
        const raw = fd.get(`metric_${s.key}`);
        if(raw !== null && raw !== '') metrics[s.key] = Number(raw) || 0;
      });
      if(Object.keys(metrics).length) saveMetricOverrideV13(program, id, getCurrentPeriod().period, metrics);
    }
    if(isPoint){ savePointV12(Object.assign({}, base, data, {id, program, lat:base.lat, lng:base.lng})); }
    else saveOverrideV12(program, id, data);
    modal.classList.add('hidden'); refreshAll(); if(STATE.map) drawMapMarkers(); toast('Información actualizada','success');
  }, {once:true});
}

function allSitesBarsV13(id, rows){
  const max = Math.max(1, ...rows.map(x=>Number(x.total)||0));
  const palette = ['#56AB2F','#8DC63F','#0E2A6B','#1E50C5','#F59E0B','#8B5CF6','#EC4899','#06B6D4'];
  const htmlRows = rows.map((x,i)=>{
    const pct = Math.max(2, (Number(x.total)||0)/max*100);
    return `<div class="native-bar-row all-site-row"><div class="native-bar-label" title="${escapeHtml(x.name)}">${escapeHtml(x.name)}</div><div class="native-bar-track"><div class="native-bar-fill" style="width:${pct}%;background:${palette[i%palette.length]}"></div></div><div class="native-bar-value">${fmt(x.total)}</div></div>`;
  }).join('');
  setChart(id, `<div class="native-chart native-bars native-bars-all-sites">${htmlRows}</div>`);
}
renderTopChart = function(period){
  const head = chartHost('topChart')?.closest('.chart-card')?.querySelector('.chart-head h3');
  if(head) head.textContent = 'Todas las sedes por atenciones';
  const rows = activeModules().map(mod0=>{
    const mod = mergeItemV12('salud', mod0, mod0.id);
    return {name:mod.name || mod0.name, total:totalOf(period.modules?.[mod0.id]||{})};
  }).sort((a,b)=>b.total-a.total);
  allSitesBarsV13('topChart', rows);
};

const _renderSportsOverviewV13 = renderSportsOverview;
renderSportsOverview = function(){
  _renderSportsOverviewV13();
  const head = chartHost('statusChart')?.closest('.chart-card')?.querySelector('.chart-head h3');
  if(head) head.textContent = 'Estructura deportiva';
  const units = sportsUnits();
  const committees = sportsCommittees();
  const summary = sportsSummary();
  nativeDonut('genderChart', ['Comités','Administradores'], [summary.committeeCount||0, summary.adminCount||0], ['#56AB2F','#8DC63F']);
  nativeDonut('statusChart', ['Unidades deportivas','Comités / canchas'], [summary.units||units.length, summary.committees||committees.length], ['#F59E0B','#56AB2F']);
};

const _renderSportsModulesV13Base = renderSportsModules;
renderSportsModules = function(){
  _renderSportsModulesV13Base();
  $$('.module-deporte, .module-comite').forEach(card=>{
    // Armonía visual: evita acentos azules en tarjetas deportivas añadidas por versiones previas.
    card.classList.add('sport-harmony-v13');
  });
};
